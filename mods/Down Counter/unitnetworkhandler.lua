...
Hooks:Add("NetworkReceivedData", "NetworkReceivedData_patchy", function(sender, message, data, ...)
	if message == DownCounter.ModID then
		...
		if LuaNetworking:LocalPeerID() < DownCounter.king then 
			DownCounter.king = LuaNetworking:LocalPeerID()
			LuaNetworking:SendToPeer(peer_id,_G.DownCounterStandalone.ModID, "Robot Democracy Lives! I shall be and have always been the once and future king!")
		end
		
		if sender < DownCounter.king then
			DownCounter.king = sender
		end
		
		--do nothing if king has lower peerid
	end
end)
